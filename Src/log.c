...
#define RES_10000       (2)
#define RES_100000      (3)
#define RES_1000000     (4)

#define RESOLUTION RES_100000
...
void eMPL_send_dwm(float **anchorState, int noAnchors, float *tagState){



    char out[PACKET_LENGTH];
    int i;
    if (!anchorState && noAnchors > 0)
        return;
    ...
    int length = (noAnchors * 4 * 3) + (4 * 3);

    length = (length > PACKET_LENGTH)? length : PACKET_LENGTH;

    int res;

    #if RESOLUTION == RES_1000
        res = 1000;
    #elif RESOLUTION == RES_10000
        res = 10000;
    #elif RESOLUTION == RES_100000
        res = 100000;
    #elif RESOLUTION == RES_1000000
        res = 1000000;
    #endif
    ...
    // X position
    out[4] = (char)((int)(tagState[0]*res) >> 24);
    out[5] = (char)((int)(tagState[0]*res) >> 16);
    out[6] = (char)((int)(tagState[0]*res) >> 8);
    out[7] = (char)(int)(tagState[0]*res);

    // Y position
    out[8] = (char)((int)(tagState[1]*res) >> 24);
    out[9] = (char)((int)(tagState[1]*res) >> 16);
    out[10] = (char)((int)(tagState[1]*res) >> 8);
    out[11] = (char)(int)(tagState[1]*res);

    // Z position
    out[12] = (char)((int)(tagState[2]*res) >> 24);
    out[13] = (char)((int)(tagState[2]*res) >> 16);
    out[14] = (char)((int)(tagState[2]*res) >> 8);
    out[15] = (char)(int)(tagState[2]*res);


    for(i = 0; i < noAnchors; i++){
        // X position
        out[16+(i*(3*4))] = (char)((int)(anchorState[(i*3)][0]*res) >> 24);
        out[17+(i*(3*4))] = (char)((int)(anchorState[(i*3)][0]*res) >> 16);
        out[18+(i*(3*4))] = (char)((int)(anchorState[(i*3)][0]*res) >> 8);
        out[19+(i*(3*4))] = (char)(int)(anchorState[(i*3)][0]*res);

        // Y position
        out[20+(i*(3*4))] = (char)((int)(anchorState[(i*3)+1][0]*res) >> 24);
        out[21+(i*(3*4))] = (char)((int)(anchorState[(i*3)+1][0]*res) >> 16);
        out[22+(i*(3*4))] = (char)((int)(anchorState[(i*3)+1][0]*res) >> 8);
        out[23+(i*(3*4))] = (char)(int)(anchorState[(i*3)+1][0]*res);

        // Z position
        out[24+(i*(3*4))] = (char)((int)(anchorState[(i*3)+2][0]*res) >> 24);
        out[25+(i*(3*4))] = (char)((int)(anchorState[(i*3)+2][0]*res) >> 16);
        out[26+(i*(3*4))] = (char)((int)(anchorState[(i*3)+2][0]*res) >> 8);
        out[27+(i*(3*4))] = (char)(int)(anchorState[(i*3)+2][0]*res);
    }
